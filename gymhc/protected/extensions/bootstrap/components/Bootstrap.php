...
class Bootstrap extends CApplicationComponent
{
	...
	/**
	 * Registers the Flat-UI CSS.
	 */
	public function registerFlatUiCss()
	{
		Yii::app()->clientScript->registerCssFile($this->getAssetsUrl().'/css/flat-ui.css');
	}
	...
	protected function registerJS($position = CClientScript::POS_END)
	{
		...
		$cs->registerScriptFile($this->getAssetsUrl().'/js/jquery-ui-1.10.3.custom.min.js', $position);
		$cs->registerScriptFile($this->getAssetsUrl().'/js/jquery.ui.touch-punch.min.js', $position);
		$cs->registerScriptFile($this->getAssetsUrl().'/js/bootstrap.min.js', $position);

		/* Custom scripts for Bootstrap */
		$cs->registerScriptFile($this->getAssetsUrl().'/js/bootstrap-select.js', $position);
		$cs->registerScriptFile($this->getAssetsUrl().'/js/bootstrap-switch.js', $position);

		/* Custom scripts for Flat-UI */
		$cs->registerScriptFile($this->getAssetsUrl().'/js/flatui-checkbox.js', $position);
		$cs->registerScriptFile($this->getAssetsUrl().'/js/flatui-radio.js', $position);

		/* Custom scripts for JQuery */
		$cs->registerScriptFile($this->getAssetsUrl().'/js/jquery.tagsinput.js', $position);
		$cs->registerScriptFile($this->getAssetsUrl().'/js/jquery.placeholder.js', $position);
		$cs->registerScriptFile($this->getAssetsUrl().'/js/jquery.stacktable.js', $position);

		/* Custom UI related JS */
		$cs->registerScriptFile($this->getAssetsUrl().'/js/application.js', $position);
	}
